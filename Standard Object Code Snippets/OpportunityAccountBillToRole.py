def custom_object_mappings(self):
    ...
    # OBJECT DEFINITION FOR OPPORTUNITY ACCOUNT BILL TO ROLE (CONTACT)
    # Get Opportunity Account Id
    oppAccountId = ScriptExecutor.Execute("CPQ_SF_ACCOUNT_TAG", {"PROPERTY": "Id"})
    if oppAccountId:
        mapping = dict()
        condition = "AccountId='{oppAccountId}' AND Role='Bill To'".format(oppAccountId=str(oppAccountId))
        soql = self.build_soql_query(   selectedFields="ContactId", 
                                            table="AccountContactRole",
                                            condition=condition)
        mapping["Name"] = "Opportunity_Account_Bill_To_Role"
