def custom_object_mappings(self):
    ...
    # OBJECT DEFINITION FOR OPPORTUNITY ACCOUNT SHIP TO ROLE (CONTACT)
    # Get Opportunity Account Id
    oppAccountId = ScriptExecutor.Execute("CPQ_SF_ACCOUNT_TAG", {"PROPERTY": "Id"})
    if oppAccountId:
        mapping = dict()
        condition = "AccountId='{oppAccountId}' AND Role='Ship To'".format(oppAccountId=str(oppAccountId))
        soql = self.build_soql_query("ContactId", "AccountContactRole", condition)
        mapping["Name"] = "Opportunity_Account_Ship_To_Role"
