...
def custom_object_mappings(self):
    ...
    # OBJECT DEFINITION FOR OPPORTUNITY SHIP TO ROLE (CONTACT)
    # Get Opportunity Id
    oppId = get_quote_opportunity_id(self.Quote)
    if oppId:
        mapping = dict()
        condition = "OpportunityId='{opportunityId}' AND Role = 'Ship To'".format(opportunityId=str(oppId))
        soql = self.build_soql_query("ContactId", "OpportunityContactRole", condition)
        mapping["Name"] = "Opportunity_Ship_To_Role"
