...
def custom_object_mappings(self):
    ...
    # OBJECT DEFINITION FOR OPPORTUNITY PARTNER ROLE ACCOUNT SHIP TO (ACCOUNT)
    # Get Opportunity Id
    oppId = get_quote_opportunity_id(self.Quote)
    if oppId:
        mapping = dict()
        condition = "OpportunityId='{oppId}' AND And Role = 'Ship To'".format(oppId=str(oppId))
        soql = self.build_soql_query(selectedFields="AccountToId",
                                        table="Partner",
                                        condition=condition)
        mapping["Name"] = "Opportunity_Partner_Role_Account_Ship_To"
